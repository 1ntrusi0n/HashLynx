...
The application materializes this licensed embedded asset under its own `mask-presets` data directory. In the normal application this is `%LOCALAPPDATA%\HashLynx\mask-presets`. Earlier experiment assets remain in their separate workspace. The canonical asset repairs changes to its managed cache; it never rewrites user-selected mask files or the backend installation. There is no runtime download, Python dependency, or transmission of user data.
